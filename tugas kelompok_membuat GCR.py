...
def input_multibaris(prompt_awal):
    """Memungkinkan pengguna memasukkan teks multibaris. Input diakhiri dengan baris kosong atau SELESAI."""
    print(prompt_awal)
    print(" (Ketik SELESAI di baris baru dan tekan Enter untuk mengakhiri input):")
    
    lines = []
    while True:
        try:
            line = input()
            if line.strip().upper() == 'SELESAI' or not line.strip():
                break
            lines.append(line)
        except EOFError:
            break
...
def masuk():
    ...
    if email in pengguna and pengguna[email]["password"] == password:
        print(f"Berhasil login sebagai {pengguna[email]['nama']}.")
        return email
    else:
...
def buat_kelas(email):
    ...
    print("BUAT KELAS")
    garis()
    kode = input_non_kosong("Kode kelas (contoh: IF001): ")
    if kode in kelas_data:
        print("Kode kelas sudah terpakai.")
        return
    ...
    print(f"Kelas '{nama}' berhasil dibuat dengan kode: {kode}")
...
def lihat_nilai_semua(kode):
    ...
    print("-" * (len("| " + " | ".join([h.ljust(15) for h in header]) + " |") + 2))
    
    for email_murid in murid_di_kelas:
        baris = [pengguna[email_murid]['nama'][:15].ljust(15)]
        for kt in kode_tugas_list:
            nilai = tugas_all[kt]['nilai'].get(email_murid, "-")
            baris.append(str(nilai).center(15))
        print("| " + " | ".join(baris) + " |")
...
def lihat_detail_kelas_murid(email):
    ...
    if info['tugas']:
        for kt, tugas_info in info['tugas'].items():
            ...
            print(f"  Deskripsi: {tugas_info.get('deskripsi', 'Tidak ada deskripsi.')}") 
...
def mengerjakan_tugas(email, kode):
    ...
    for kt, info in tugas_all.items():
        if email not in info["nilai"]: # Hanya tampilkan jika belum dinilai
            status = "Sudah Diserahkan" if email in info["jawaban"] else "Belum Dikerjakan"
            tugas_untuk_dikerjakan.append(kt)
            print(f"- {kt} | {info['nama']} (Status: {status})")

    if not tugas_untuk_dikerjakan:
        print("Semua tugas sudah dinilai atau belum ada tugas.")
        return

    kode_tugas = input_non_kosong("Masukkan kode tugas yang akan dikerjakan/diserahkan: ")

    if kode_tugas not in tugas_all or kode_tugas not in tugas_untuk_dikerjakan:
        print("Kode tugas tidak valid atau sudah selesai dinilai.")
        return
    
    # --- PENGGUNAAN FUNGSI MULTI-BARIS ---
    tugas_info = tugas_all[kode_tugas]
    print(f"\n--- Mengerjakan Tugas: {tugas_info['nama']} ---")
    print(f"Deskripsi: {tugas_info['deskripsi']}")
    print("-" * 40)
...
def menu_kelas_pengajar(email, kode):
    while True:
        ...
        print("0. Kembali/ Logout") # TEKS DIUBAH
        max_pilihan = 4
        pilih = input_pilihan_menu(max_pilihan)
        ...
        if pilih == "1":
            ...
            aksi = navigasi_kembali_atau_logout() # Panggil navigasi
            if aksi == 'LOGOUT':
                return 'LOGOUT'
            elif aksi == 'KEMBALI':
                break
...
def sub_menu_tugas_pengajar(kode):
    while True:
        ...
        garis()
        print("1. Tambah Tugas (CREATE + Deskripsi)")
...
def menu_pengajar(email):
    while True:
        ...
        if pilih == "1":
            ...
            konfirmasi = input("Yakin ingin Logout ke Menu Utama Sistem GCR? (Y/T): ").upper()
            if konfirmasi == 'Y':
                break
...
def menu_murid(email):
    while True:
        ...
        print("0. Logout ke Menu Utama Sistem GCR")
        max_pilihan = 4
        pilih = input_pilihan_menu(max_pilihan)
        if pilih == "1":
            lihat_kelas_murid(email)
