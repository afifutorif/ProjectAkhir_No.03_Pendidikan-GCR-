...
def input_multibaris(prompt_awal):
    """
    Memungkinkan pengguna memasukkan teks multibaris. 
    Input diakhiri hanya ketika mengetik '0' (nol) di baris baru dan Enter.
    """
    print(prompt_awal)
    print(" (Ketik 0 di baris baru dan tekan Enter untuk mengakhiri input):")
    
    lines = []
    while True:
        try:
            # Menggunakan input() tanpa argumen prompt di dalam loop
            line = input()
            
            # Cek jika pengguna mengetik '0' (nol) secara eksplisit untuk mengakhiri
            if line.strip() == '0': 
                break
    ...
    # Hapus baris kosong yang mungkin tersisa di akhir
...
def tambah_tugas(kode):
    ...
    print("\n--- Deskripsi Tugas ---")
    # GURU MENGGUNAKAN INPUT MULTI-BARIS
...
def sub_menu_tugas_pengajar(kode):
    while True:
        ...
        print("3. perbarui Tugas (UPDATE)")
        print("0. Kembali/ Logou")
        max_pilihan = 2
        pilih = input_pilihan_menu(max_pilihan)
...
def menu_pengajar(email):
    while True:
        ...
        if pilih == "1":
            ...
            konfirmasi = input("Apa Kamu Yakin ingin Logout? (Y/T): ").upper()
            if konfirmasi == 'Y':
                break
